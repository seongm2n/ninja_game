...
class Hero {
    constructor(el){
        ...
        this.maxExp = 3000;
        this.expProgress = 0;
      // 화면높이 : console.log(window.innerHeight); 
      // 히어로높이 : console.log(this.el.getBoundingClientRect().top); 
      // 히어로 탑위치 : console.log(window.innerHeight - this.el.getBoundingClientRect().top);
      // 히어로 바텀위치 : console.log(window.innerHeight - this.el.getBoundingClientRect().top - this.el.getBoundingClientRect().height); 
    }
    keyMotion(){
        if(key.keyDown['left']){
            this.direction = 'left';
    ...
    updateExp(exp){
        ...
        // console.log('현재경험치: ' + this.expProgress);
...
class Monster {
    ...
    updateHp(index){
        ...
        this.progress = this.hpValue / this.defaultHpValue *100; // 백분율구하기
